...
const apiUrl = `/iron/session/${form.value.id}/bulk-edit/`;

const submitSession = async () => {
  try {
    for (const exercise of form.value.session_exercises) {
      exercise.exercise = exercise.exercise.id;
    }

    const sessionRes = await axios.post(apiUrl, form.value);
    emit("submit", sessionRes.data);
  } catch (error) {
